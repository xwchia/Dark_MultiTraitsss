"""
Run MentalBench-Align Evaluation for Multi-Turn Crisis Conversations
...
from .mentalbench_evaluator import MentalBenchEvaluator, generate_summary
...
def run_mentalbench_evaluation(
    ...
    """
    Run MentalBench-Align 7-attribute evaluation on multi-turn conversation responses.
    
    This evaluates each turn in each conversation with cumulative context,
    ...
    Args:
        responses_path: Path to responses.json from conversation runner
        ...
        max_concurrent: Max concurrent API calls (default: 5 for multi-turn)
        checkpoint_interval: Save checkpoint every N conversations
        resume: Resume from checkpoint if exists
    
    Outputs:
        - mentalbench_evaluations.csv: Per-turn scores for all 7 attributes + composites
    ...
    """
    print("=" * 60)
    print("MENTALBENCH-ALIGN EVALUATION (Multi-Turn)")
    print("=" * 60)
    ...
    # Count total turns
    total_turns = sum(len(conv.get("turns", [])) for conv in results)
    print(f"   Total turns to evaluate: {total_turns}")
    ...
    if resume and checkpoint_path.exists():
        ...
        print(f"   📥 Resuming from checkpoint: {len(existing_evals)} turn evaluations already completed")
